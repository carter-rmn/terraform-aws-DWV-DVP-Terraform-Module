...
resource "aws_secretsmanager_secret_version" "dwv_secret_terraform" {
  ...
  secret_string = jsonencode({
    mongo = {
      dw_core = {
        name        = local.databases.mongo.dw_core.name
        ...
        users = {
          root = {
            username = local.databases.mongo.dw_core.usernames.root
            password = random_password.mongo_dw_core_root_password.result
          }
          app = {
            username          = local.databases.mongo.dw_core.usernames.app
            password          = random_password.mongo_dw_core_app_password.result
            connection_string = "mongodb://${local.databases.mongo.dw_core.usernames.app}:${random_password.mongo_dw_core_app_password.result}@${join(",", [for item in aws_instance.ec2s : "${item.private_ip}:${local.databases.mongo.port}" if length(regexall("(mongo-\\d+)", item.tags.Short)) > 0])}/${local.databases.mongo.dw_core.name}?authSource=admin${length([for item in aws_instance.ec2s : 1 if length(regexall("(mongo-\\d+)", item.tags.Short)) > 0]) > 1 ? "&replicaSet=dwv" : ""}"
          }
          viewer = {
            username          = local.databases.mongo.dw_core.usernames.viewer
