resource "aws_cloudwatch_metric_alarm" "ec2s" {
  for_each            = (var.alarm.enabled && var.CREATE_IAM) ? { for alarm in local.alarm.ec2 : alarm.name => alarm } : {}
...
resource "aws_cloudwatch_metric_alarm" "lambda" {
  for_each            = (var.alarm.enabled && var.CREATE_IAM) ? { for alarm in local.alarm_config.lambda : alarm.alarm_name => alarm } : {}
